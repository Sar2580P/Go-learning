...
const defaultHoldTTL= 2*time.Minute    // TTL: time to leave

var _ BookingStore=(*RedisStore)(nil)
/*
Key design:

seat:{movieID}:{seatID} 		-> session JSON (TTL=held, no TTL=confirmed) 
session:{sessionID} 			-> seat key (reverse lookup for cleanup)
*/
...
func (s *RedisStore) ListBookings(movieID string) []Booking {
	pattern:= fmt.Sprintf("seat:%s:*", movieID)  // fetch all seats: key --> "seat:%s:%s", b.MovieID, b.SeatID
...
func (s *RedisStore) getSession(ctx context.Context, sessionID string, userID string) (Booking, string, error) {
	...
	// actual Redis key where the booking data is stored.
	// Returns the seat key (e.g., "seat:movieA:seat5")
	sk, err := s.rdb.Get(ctx, sessionKey(sessionID)).Result()
	if err != nil {
		return Booking{}, "", err
	}

	// Uses that seat key to fetch the actual booking JSON
